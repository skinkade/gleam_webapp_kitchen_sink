import backend/models/user.{type UserId}
...
pub fn create_with_defaults(
  ...
  let sql =
    "
        ...
        (token_hash, user_id, created_at, expired_at)
        VALUES
        ($1, $2, $3, $4);
    "
...
pub fn get_by_session_key_string(
  ...
  use result <- result.try({
    traced.sql(sql, conn, [hash |> pgo.bytea()], decode_session_record)
  })
...
pub fn is_expired(session: SessionQueryRecord) -> Bool {
  case birl.compare(birl.utc_now(), session.expired_at) {
    order.Gt -> True
    _ -> False
  }
}
